...
class _WatchHomeScreenState extends State<WatchHomeScreen> {
  ...
  Future<void> _initializeDataCollector() async {
    try {
      ...
      if (initialized) {
        ...
        _dataCollector.healthDataStream.listen((healthData) {
          if (mounted) {
            setState(() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // 변화율
          _buildInfoRow(
            icon: _currentBattery!.changeRate > 0 
              ? Icons.trending_up 
              : Icons.trending_down,
            label: '변화율',
            value: '${_currentBattery!.changeRate > 0 ? '+' : ''}${_currentBattery!.changeRate.toStringAsFixed(1)}%/h',
            color: _currentBattery!.changeRate > 0 
              ? Colors.green 
              : Colors.orange,
          ),
  ...
  String _getEstimatedTime() {
    ...
    final hours = _currentBattery!.changeRate > 0
